...
- `gShowQuickView` / `gQuickViewRequest` — `HomeScreen`'s **Quick View** lightbox (`rectQuickViewOverlay_HS` / `conQuickViewDialog_HS`, the **last two screen-level children** — z-order in Canvas is child order, so they must stay last). Each gallery row's `btnQuickView` (first control in `rowActions`) does `Set(gQuickViewRequest, ThisItem); Set(gShowQuickView, true)`; the dialog reads `gQuickViewRequest` — a **separate var from `gSelectedRequest`** so opening the popup doesn't mutate the navigation target, and its "Open Full Detail" button is what copies it into `gSelectedRequest` before navigating. Shows Title, Category, ProcurementType, ProcurementDescription (scrolling label, the only `FillPortions: =1` child), `RequesterInvoiceURL` (gated on `ProcurementType = "Invoice Supplied"`) and `OfficialInvoiceLink`. `gShowQuickView` is reset to `false` in `HomeScreen.OnVisible` and seeded in `App.OnStart`. This popup replaced the per-row `lblRequestDesc` label, which is why the gallery row shrank from 130 to 114 (`galRequests.TemplateSize`, `rowRequestContent.Height`, `rectRowHitArea.Height` 82, `rectSeparator.Y` 113 — change all five together). The layout mirrors the sibling `procurement-raw-materials` app's `HomeScreen` control-for-control; keep the two in sync when either changes.
