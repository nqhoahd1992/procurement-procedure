...
- `gProjectInfo` / `colProjectInvoices` / `gShowRateInfo` — set in `ManagerReviewScreen.OnVisible` and `ExecutiveApprovalScreen.OnVisible` (**not** in `App.OnStart`), backing the "Project Information" panel: `gProjectInfo` is the `LookUp(Project_List, ProjectID = gSelectedRequest.ProjectID)` row (blank when the request isn't tied to a project), `colProjectInvoices` the matching `Procurement_InvoiceData` rows. On `ManagerReviewScreen` the panel is one compact 116px row — `rowProjectFields_MR` holds four inline columns (Related Project — the looked-up `Project_List.Title`, not the raw `ProjectID` — Budget Amount, Grand Total, Project Short Description), matching `procurement-raw-materials`' `ManagerReviewScreen`; the earlier taller layout (separate "Project ID"/"Actual Cost (invoiced to date)" columns, a divider, then Grand Total and a "View exchange rates" link each on their own row) was dropped for this screen to match. `gShowRateInfo` (the "Currency Exchange Reference" dialog toggle, reset to `false` on every screen entry) and its `rectRateInfoOverlay_MR`/`conRateInfoDialog_MR` are still declared here but now have no trigger on `ManagerReviewScreen` since the "View exchange rates" link was removed with it — dead code kept only because `procurement-raw-materials`' own copy of this screen is in the same state (its link is gone too, with nothing left to open the dialog). The AUD conversion rates behind the panel's Grand Total are hardcoded in a `Switch(Upper(Currency), ...)` inside the `lblProjectGrandTotal_MR`/`_EA` labels and mirrored as plain text in the (now-unreachable-from-here) dialog — change both together, and keep them in sync with the same table in `project-list`'s `ViewProjectScreen` and in the sibling `procurement-raw-materials` app. The **same rate table** is now also duplicated in `RequestFormScreen`'s 5,000-AUD-equivalent escalation check (6 occurrences) and `ExecutiveApprovalScreen`'s manager-skipped-reason label — when rates change, update all of these together too.
