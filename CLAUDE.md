...
- `gProjectInfo` / `colProjectInvoices` / `gShowRateInfo` — set in `ManagerReviewScreen.OnVisible` and `ExecutiveApprovalScreen.OnVisible` (**not** in `App.OnStart`), backing the "Project Information" panel: `gProjectInfo` is the `LookUp(Project_List, ProjectID = gSelectedRequest.ProjectID)` row (blank when the request isn't tied to a project), `colProjectInvoices` the matching `Procurement_InvoiceData` rows. On both screens the panel is one compact 116px row — `rowProjectFields_MR`/`_EA` holds four inline columns (Related Project — the looked-up `Project_List.Title`, not the raw `ProjectID` — Budget Amount, Grand Total, Project Short Description), matching `procurement-raw-materials`' copies of these screens; the earlier taller layout (separate "Project ID"/"Actual Cost (invoiced to date)" columns, a divider, then Grand Total and a "View exchange rates" link each on their own row) was dropped from both to match. `gShowRateInfo` (the "Currency Exchange Reference" dialog toggle, reset to `false` on every screen entry) and each screen's `rectRateInfoOverlay_*`/`conRateInfoDialog_*` are still declared but now have no trigger on either screen since the "View exchange rates" link was removed from both — dead code kept only because `procurement-raw-materials`' own copies of these two screens are in the same state (their links are gone too, with nothing left to open the dialog). The AUD conversion rates behind the panel's Grand Total are hardcoded in a `Switch(Upper(Currency), ...)` inside the `lblProjectGrandTotal_MR`/`_EA` labels and mirrored as plain text in the (now-unreachable-from-here) dialogs — change both together, and keep them in sync with the same table in `project-list`'s `ViewProjectScreen` and in the sibling `procurement-raw-materials` app. The **same rate table** is now also duplicated in `RequestFormScreen`'s 5,000-AUD-equivalent escalation check (6 occurrences) and `ExecutiveApprovalScreen`'s manager-skipped-reason label — when rates change, update all of these together too.
...
- **Status badge colors** — every status pill/badge in the app (`HomeScreen`'s gallery status label, `RequestDetailScreen`'s header badge, and each work screen's `lblStatusBadge_*`) uses the same `Switch(<request>.Status.Value, ...)` pair for `Color`/`BorderColor` and `Fill`, one distinct color per status, matching `procurement-raw-materials`':

  | Status | Color | Fill |
  |---|---|---|
  | Pending Manager | `RGBA(146, 84, 14, 1)` | `RGBA(253, 242, 216, 1)` |
  | Pending Executive | `RGBA(175, 74, 20, 1)` | `RGBA(253, 234, 222, 1)` |
  | Pending Procurement | `RGBA(122, 108, 10, 1)` | `RGBA(247, 245, 210, 1)` |
  | Pending Invoice | `RGBA(124, 58, 173, 1)` | `RGBA(240, 228, 252, 1)` |
  | Goods Receipt & Acceptance | `RGBA(37, 92, 178, 1)` | `RGBA(223, 234, 252, 1)` |
  | Pending Supplier Follow-up | `RGBA(11, 105, 132, 1)` | `RGBA(219, 241, 249, 1)` |
  | Pending Accounting | `RGBA(158, 44, 108, 1)` | `RGBA(252, 230, 242, 1)` |
  | Completed | `RGBA(15, 110, 86, 1)` | `RGBA(222, 245, 236, 1)` |
  | Rejected | `RGBA(163, 45, 45, 1)` | `RGBA(252, 233, 233, 1)` |
  | *(fallback)* | `RGBA(68, 68, 65, 1)` | `RGBA(241, 239, 232, 1)` |

  Work screens whose badge previously showed a fixed single color (because the screen is only ever reached at one status) now use this same full `Switch` too, rather than a hardcoded color — it's simpler to keep one shared table than to reason about which screens are truly single-status, and it stays correct if that ever stops being true. The "Pending Payment From Executive" label (`ExecutivePaymentScreen`, and the computed sub-status text elsewhere) keys its color off the *real* `Status.Value`, which is still `"Pending Executive"` — it gets that status's color, not a separate one. This table is unrelated to the amber `RGBA(133, 79, 11, 1)` used elsewhere in the app for warning/attention banners (escalation notices, "not authorized" boxes, conditional-approval flags) — those are a different semantic and were left alone. When adding a new status or changing one of these colors, update every occurrence together.
- **The header bar and the bottom action bar are both fixed on every screen — only the content between them scrolls.** Every screen but `HomeScreen` follows the same shell: a single `Screen.Children` list of `[rowHeader_*, conScrollable_*/containerScroll_*, conActionBar_*, …any overlay dialogs]`. The purple app-title bar (background + white title Label + white user-info Label — these used to be three separate fixed screen-level children, with the user-info one often declared far away in the file, e.g. after `conActionBar_*`) is merged into one `rowHeader_*` `AutoLayout` `GroupContainer` (`Fill: =RGBA(83, 74, 183, 1)`, `Height: =60`, horizontal, `LayoutJustifyContent: =LayoutJustifyContent.SpaceBetween`, 20px left/right padding) and is a **fixed top-level screen child**, immediately before the scroll container — that three-into-one consolidation is a real cleanup worth keeping even though the fixed positioning itself reverted. The scroll container (`conScrollable_*`/`containerScroll_*`) has `Y: =60` and `Height: =Parent.Height - 120` (60 for the fixed header, 60 for the fixed action bar) and holds everything else — the request-info block (`rowInfoBlock_*`/`rowTitle`), forms, checklists, whatever the screen's actual content is. The action bar (`conActionBar_*`/`rowButtons`, `Height: =60`, `Y: =Parent.Height - 60`) and any overlay dialogs (rate-info, receipt-photo, log-detail) stay fixed screen-level children, in their original order. **`HomeScreen` is the deliberate exception** — its purple header + toolbar (Status filter, New Request) stay fixed above the gallery as three separate controls (never consolidated), because it has no action bar to anchor against and the user asked to leave it as-is; only its gallery scrolls, the same as before.
