...
- **Only the bottom action bar is fixed on any screen — the header scrolls away with everything else.** Every screen but `HomeScreen` follows the same shell: a single `Screen.Children` list of `[conScrollable_*/containerScroll_* , conActionBar_*, …any overlay dialogs]`. The purple app-title bar (background + white title Label + white user-info Label, formerly three separate fixed screen-level children — the user-info one was often declared far away in the file, e.g. after `conActionBar_*`) is merged into one `rowHeader_*` `AutoLayout` `GroupContainer` (`Fill: =RGBA(83, 74, 183, 1)`, `Height: =60`, horizontal, `LayoutJustifyContent: =LayoutJustifyContent.SpaceBetween`, 20px left/right padding) and is the scroll container's **first child**, not a fixed sibling above it. Any other block that used to sit fixed between the header and the scroll area (a beige/lavender `rowInfoBlock_*`/`rowTitle` request-summary strip) is folded in right after `rowHeader_*`, before the screen's own content. The scroll container itself has no `Y` (starts at the screen top) and `Height: =Parent.Height - 60` (only the fixed action bar subtracted). The action bar (`conActionBar_*`/`rowButtons`, `Height: =60`, `Y: =Parent.Height - 60`) and any overlay dialogs (rate-info, receipt-photo, log-detail) are untouched by this and stay fixed screen-level children, in their original order. **`HomeScreen` is the deliberate exception** — its purple header + toolbar (Status filter, New Request) stay fixed above the gallery, because it has no action bar to anchor against and the user asked to leave it as-is; only its gallery scrolls, the same as before.
