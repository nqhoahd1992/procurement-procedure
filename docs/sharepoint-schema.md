...
| `ManagerApproverNum` | Number | Plain numeric mirror of `ManagerApproverID.Id`, written alongside it, purely so `HomeScreen` can filter by it delegably (see note above) |
...
| `GRAssignedToNum` | Number | Plain numeric mirror of `GRAssignedToID.Id`, written alongside it, purely so `HomeScreen` can filter by it delegably (see note above) |
| `SFU1AssignedToID` | Lookup→Employee List | delegate for Step 4 Supplier Follow-up (Requester); blank = Requester performs it |
| `SFU1AssignedToNum` | Number | Plain numeric mirror of `SFU1AssignedToID.Id`, written alongside it, purely so `HomeScreen` can filter by it delegably (see note above) |
...
**`colProcurementUsers`** — a snapshot of `Procurement_User` (`ClearCollect(colProcurementUsers, Filter(Procurement_User, IsActive))`, once in `App.OnStart`). Every screen that needs to query this list by a Lookup column's sub-field does it against this collection instead of the live SharePoint table, because `EmployeeID.Id`/`EmployeeID.Value` aren't delegable (same reasoning as the plain-Number mirror columns on `Procurement_Requests` — `ManagerApproverNum`/`GRAssignedToNum`/`SFU1AssignedToNum` — just solved here by snapshotting the table instead of adding a mirror column). The `Filter(..., IsActive)` is both the App Checker's literal fix for "rows retrieved may not be complete" (a bare `ClearCollect(col, DataSource)` with no filter is flagged regardless of table size) and a real narrowing — an employee whose role row has `IsActive = false` now resolves to the default role `Requester` in `gCurrentUser` instead of their stale role, which wasn't true before this fix. If a former Manager/Executive/etc. is expected to keep authority via a *deactivated* row (unlikely — `IsActive` exists specifically to retire a role assignment), flag that before relying on this.
- `App.OnStart` resolves `gCurrentUser` via `LookUp(colProcurementUsers, EmployeeID.Id = gCurrentEmployee.ID)`. Employees absent here (never had a role row, or their row is now filtered out by `IsActive`) default to role `Requester`; role checks elsewhere read `gCurrentUser.Role.Value` directly.
- `RequestFormScreen`'s `ddManagerApprover_1.Items` does `Sort(Filter(colProcurementUsers, Role.Value = "Manager" && IsActive), EmployeeID.Value, SortOrder.Ascending)` — `Role.Value`/`IsActive` are delegable on the live table, but sorting by `EmployeeID.Value` isn't, so the whole formula runs against the snapshot. (The `IsActive` filter here is now redundant with the snapshot already being pre-filtered, but harmless — kept for clarity in case the snapshot's own filter ever changes.)

If active rows in `Procurement_User` ever grow past the app's non-delegable row cap (default 500, raisable to 2,000 in Studio's General settings — a setting that lives in `Properties.json`, which this repo intentionally doesn't keep, so it can't be set from source), this snapshot silently misses rows beyond the cap. The structural fix — a plain Number mirror column on `EmployeeID` (e.g. `EmployeeIDNum`), filtered directly on the live table — would remove the cap entirely, matching the pattern already used on `Procurement_Requests`; it hasn't been done here because it requires a SharePoint schema change (add + backfill the column) outside this app's Power Fx source.
